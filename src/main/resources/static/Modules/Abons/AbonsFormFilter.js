export class AbonsFormFilter extends FormView {
    constructor() {
        super();
    ...
    Show(filter) {
        ...
        LoadForm("#ModalWindows", this.GetUrl("/Abons/AbonsFormFilter"), this.InitFunc.bind(this));
    }
     ...
     */
    InitFunc() {
        this.InitComponents("wAbonsFormFilter_Module_Abons", ""); //Автоматическое получение идентификаторов формы
        this.InitCloseEvents(this.wAbonsFormFilter);//Инициализация закрытия формы по нажатию на клавиши "ESC" и "Enter"

        this.btnOk.linkbutton({onClick:this.btnOk_onClick.bind(this)});
        this.btnCancel.linkbutton({onClick:()=>{this.wAbonsFormFilter.window("close")}});//Обработка события нажатия на кнопку отмены
    ...
    btnOk_onClick(){
        ...
        //this.sourceFilter.showDelAbon = this.cbShowDel.checkbox('options').checked;

        if (this.ResultFunc != null) {
            this.ResultFunc();
            this.wAbonsFormFilter.window("close")
        }
    }
